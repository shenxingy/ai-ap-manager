...
async def _upsert_exception(
    db: AsyncSession, invoice: Invoice, exception_type: str,
    ...
    if result.scalars().first():
        return None
    exc = ExceptionRecord(
        invoice_id=invoice.id, exception_type=exception_type,
        severity=severity, description=description, status=status,
    )
    db.add(exc)
    await db.flush()
    print(f"  [new]  Exception {exception_type} for {invoice.invoice_number}")
    return exc
...
async def _upsert_approval_task(
    ...
    if result.scalars().first():
        return None
    task = ApprovalTask(
        invoice_id=invoice.id, assigned_to=approver.id,
        step_order=1, status=status, required_count=1,
        notes=notes,
